...
function ytBtn(query,label){
  return `<a href="${ytLink(query)}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:5px;background:var(--surf,var(--card));color:var(--red);border:1px solid var(--red-l);font-size:11px;font-weight:500;padding:4px 10px;border-radius:12px;text-decoration:none;margin-top:6px" class="subtopic-clip-btn">▶ ${label||'ดูคลิปติว'}</a>`;
}
...
<link href="https://fonts.googleapis.com/css2?family=Anuphan:wght@300;400;500;600;700&family=Space+Grotesk:wght@400;500;600;700&display=swap" rel="stylesheet">
<style>
*{box-sizing:border-box;margin:0;padding:0;font-family:var(--font-thai)}
body{padding:28px 30px;color:var(--text1);font-size:13px}
.hd{border-bottom:2px solid var(--blue);padding-bottom:12px;margin-bottom:18px}
.hd h1{font-size:20px;font-weight:700;color:var(--blue)}
...
.cat-h{display:flex;justify-content:space-between;align-items:center;background:var(--surf);border-radius:6px;padding:8px 12px;font-weight:600;font-size:14px;margin-bottom:6px}
.cat-h .pct{font-size:12px;color:var(--red);font-weight:500}
table{width:100%;border-collapse:collapse}
td{padding:7px 8px;border-bottom:1px solid var(--border-md);vertical-align:top}
.chk{font-size:16px;width:24px}
.qn{font-weight:600;white-space:nowrap;width:60px}
.src{color:#444}
.lvl{color:var(--amber);white-space:nowrap;font-size:11px}
.yt a{color:var(--red);text-decoration:none;font-weight:600;font-size:12px}
...
function renderPracticePlan(d){
  ...
  cats.forEach(c=>{
    ...
    pick.forEach(q=>{ const stars='★'.repeat(q.l)+'☆'.repeat(5-q.l);
      rows+=`<div class="rev-row"><div style="flex:1"><div style="font-size:12px;color:var(--text1);font-weight:500">ข้อ ${q.n} <span style="color:var(--text3);font-weight:400">· ${q.s}</span></div><div style="font-size:10px;color:var(--text3)">ระดับ ${q.l} <span style="color:var(--amber)">${stars}</span></div></div><a href="${q.yt}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:5px;background:var(--surf,var(--card));color:var(--red);border:1px solid var(--red-l);font-size:11px;font-weight:500;padding:5px 12px;border-radius:12px;text-decoration:none;white-space:nowrap">▶ เฉลย</a></div>`; });
...
function _sparkSvg(scores,w,h){
  ...
  return `<svg viewBox="0 0 ${w} ${h}" style="width:100%;max-width:${w}px;height:${h}px;display:block"><line x1="${pad}" y1="${ty}" x2="${w-pad}" y2="${ty}" stroke="var(--red)" stroke-width="1" stroke-dasharray="4 3" opacity=".5"/><polyline points="${line}" fill="none" stroke="var(--blue)" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/><circle cx="${last[0]}" cy="${last[1]}" r="4" fill="var(--amber)"/></svg>`;
}
function renderTrendCard(d){
  ...
  const deltaColor=d.delta>0?'var(--green)':(d.delta<0?'var(--amber)':'var(--text3,var(--text3))');
  ...
  let chips='';
  if(d.isBest)chips+=chip('🏆 สูงสุดตั้งแต่เริ่มเรียน','var(--green-l)','var(--green)');
  if(d.streak>=2)chips+=chip('🔥 ดีขึ้น '+d.streak+' ครั้งติด','var(--amber-l)','var(--amber)');
  if(d.delta!=null&&d.delta<0)chips+=chip('อยู่ในช่วงผันผวนปกติ — โฟกัสที่แผนทบทวนต่อได้เลย','var(--surf)','var(--text2)');
  ...
  if(mh.length>=2){
    const cols=mh.map(h=>{
      ...
      return `<div style="text-align:center"><div style="display:flex;flex-direction:column-reverse;width:20px;border-radius:4px;overflow:hidden;margin:0 auto">${seg(h.score,'#2D6A4F')}${seg(h.care,'#F2C744')}${seg(h.concept,'#E8571F')}${seg(h.cant,'#8C2D0A')}${seg(h.timeout,'#55606B')}</div><div style="font-size:9px;color:var(--text3,var(--text3));margin-top:2px">${h.score}</div></div>`;
    }).join('');
    mixHtml=`<div style="margin-top:10px;border-top:1px dashed var(--border,var(--border-md));padding-top:8px"><div style="font-size:11px;color:var(--text3,var(--text3));margin-bottom:6px">ส่วนผสมผลรายครั้ง — 🟩 ถูก · 🟨 สะเพร่า · 🟪 คอนเซปต์ · 🟥 ทำไม่ได้ · ⬜ ไม่ทัน <span style="color:var(--text2,var(--text2))">(คะแนนเท่าเดิมแต่สีข้างบนหด = กำลังพัฒนา)</span></div><div style="display:flex;gap:9px;align-items:flex-end">${cols}</div></div>`;
  }
  card.innerHTML=`<div style="background:#fff;border:1px solid var(--border,var(--border-md));border-radius:14px;padding:14px 16px;margin:10px 0">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px">
      <div>
        <div style="font-size:11px;color:var(--text3,var(--text3))">📈 พัฒนาการของคุณ · ${hist.length} ครั้ง · เทียบกับตัวเองเท่านั้น</div>
        <div style="font-size:24px;font-weight:700;color:${deltaColor}">${deltaTxt} <span style="font-size:12px;color:var(--text3,var(--text3));font-weight:400">จากครั้งก่อน (${d.prev.score} → ${d.score})</span></div>
        <div style="margin-top:5px">${chips}</div>
      </div>
      <div style="flex:1;min-width:170px;max-width:250px">
        ${_sparkSvg(scores,250,62)}
        <div style="font-size:10px;color:var(--text3,var(--text3));text-align:right">เส้นประแดง = เป้า 25/30 · ${scores.join(' → ')}</div>
      </div>
    </div>
    ${mixHtml}
  </div>`;
...
function renderProgressTrend(d){
  ...
  trendChartInst=new Chart(document.getElementById('s-trendChart'),{
    ...
    data:{labels,datasets:[
      {label:'คะแนน',data:hist.map(h=>h.score),borderColor:'var(--blue)',backgroundColor:'rgba(24,95,165,.12)',fill:true,tension:.25,pointRadius:4,pointBackgroundColor:'var(--blue)'},
      {label:'เป้า 25',data:hist.map(()=>25),borderColor:'var(--red)',borderDash:[6,4],pointRadius:0,fill:false}
    ]},
  ...
  mixChartInst=new Chart(document.getElementById('s-mixChart'),{
    type:'bar',
    data:{labels,datasets:[
      {label:'✅ ถูก',data:hist.map(h=>h.score),backgroundColor:'#2D6A4F'},
      {label:'⚠️ สะเพร่า',data:hist.map(h=>h.care),backgroundColor:'#F2C744'},
      {label:'C คอนเซปต์',data:hist.map(h=>h.concept),backgroundColor:'#E8571F'},
      {label:'❌ ทำไม่ได้',data:hist.map(h=>h.cant),backgroundColor:'#8C2D0A'},
      {label:'⏰ ไม่ทัน',data:hist.map(h=>h.timeout),backgroundColor:'#55606B'}
    ]},
  ...
  if(tbl){
    ...
    hist.forEach((h,i)=>{
      ...
      const dtxt=dlt==null?'<span style="color:var(--text3)">—</span>':(dlt>0?'<span style="color:var(--green);font-weight:600">▲ +'+dlt+'</span>':(dlt<0?'<span style="color:var(--amber);font-weight:600">▼ '+dlt+'</span>':'▬ 0'));
...
function renderStudentDash(d){
  ...
  needReview.forEach(r=>{const statusEmoji={ok:'✅',care:'⚠️',concept:'📖',cant:'❌',timeout:'⏰',wrong:'❌',blank:'⬜'}[r.type]||'⚠️';const stars='★'.repeat(r.level)+'☆'.repeat(5-r.level);rl.innerHTML+=`<div class="rev-row"><div style="min-width:20px;font-size:11px;color:var(--text2);font-weight:500">${r.q}</div><div style="flex:1"><div style="font-size:12px;color:var(--text1)">${r.sub}</div><div style="font-size:10px;color:var(--text3)">${r.year} · ระดับ ${r.level} <span style="color:var(--amber)">${stars}</span></div></div><span style="font-size:16px">${statusEmoji}</span></div>`;});
  } // close else
  ...
  if(planEl){
    ...
    if(careQs.length)steps.push({n:steps.length+1,icon:'⚡',color:'#E8571F',title:`เก็บคะแนนจากข้อสะเพร่า (${careQs.length} ข้อ)`,
    ...
    if(conceptQs.length)steps.push({n:steps.length+1,icon:'💡',color:'#55606B',title:`เสริมคอนเซปต์พื้นฐาน (${conceptQs.length} ข้อ)`,
    ...
    if(easyWrong.length)steps.push({n:steps.length+1,icon:'📗',color:'#2D6A4F',title:`แก้ข้อผิดระดับพื้นฐาน (${easyWrong.length} ข้อ)`,
    ...
    if(easyBlank.length)steps.push({n:steps.length+1,icon:'📘',color:'var(--blue)',title:`ลองข้อที่ไม่ทัน/ไม่ได้ทำระดับพื้นฐาน (${easyBlank.length} ข้อ)`,
    ...
    if(hardOnes.length)steps.push({n:steps.length+1,icon:'🏔️',color:'var(--red)',title:`ท้าทายข้อยาก (${hardOnes.length} ข้อ)`,
    ...
    try{
      ...
      if(chronic.length){
        chronicHtml='<div style="background:var(--amber-l);border:1px solid var(--amber);border-radius:10px;padding:10px 12px;margin:2px 0 10px"><div style="font-size:12px;font-weight:600;color:var(--amber);margin-bottom:4px">🔁 จุดอ่อนเรื้อรัง — พลาดซ้ำมากกว่า 1 การสอบ</div>'
          ...
          +'<div style="font-size:11px;color:var(--amber);margin-top:6px">หัวข้อที่พลาดซ้ำแบบนี้ ควรกลับไปทำความเข้าใจแนวคิดใหม่กับครู ไม่ใช่แค่ฝึกโจทย์เพิ่มครับ (พลาดครั้งแรกในหัวข้ออื่นเป็นเรื่องปกติ ไม่ต้องกังวล)</div></div>';
      }
    ...
    if(!steps.length){planEl.innerHTML=chronicHtml+'<div style="font-size:13px;color:var(--green);padding:8px 0">ทำถูกครบทุกข้อ ไม่มีอะไรต้องทบทวน 🎉</div>';}
      ...
      let html=chronicHtml+`<div style="margin-bottom:8px"><div style="background:var(--blue-l,var(--blue-l));height:9px;border-radius:99px;overflow:hidden"><div id="sp-fill" style="height:100%;background:var(--blue);border-radius:99px;transition:width .3s;width:${Math.round(done0/steps.length*100)}%"></div></div><div id="sp-text" style="font-size:11px;color:var(--text3);margin-top:4px">ทำแล้ว ${done0} จาก ${steps.length} ขั้น${done0===steps.length?' — ครบแล้ว เก่งมาก! 🎉':''}</div></div>`;
      steps.forEach(s=>{
        ...
        <input type="checkbox" data-spkey="${spKey(s.n)}" onchange="spTick(this)" ${isDone?'checked':''} style="width:19px;height:19px;accent-color:var(--blue);margin-top:6px;cursor:pointer;flex-shrink:0">
        ...
        <div style="flex:1">
          ...
          ${s.yt?`<div style="display:flex;gap:6px;flex-wrap:wrap">${s.yt.map(t=>`<a href="${ytLink(t)}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:4px;background:var(--red);color:#fff;font-size:10px;font-weight:500;padding:3px 10px;border-radius:20px;text-decoration:none">🎬 ${t}</a>`).join('')}</div>`:''}
        </div>
      </div>`;
      });
  ...
  diffChartInst=new Chart(document.getElementById('s-diffChart'),{type:'bar',data:{labels:diffLevels.map(l=>'ระดับ '+l),datasets:[{label:'% ถูก',data:diffLevels.map(lv=>Math.round(d.diffMap[lv].ok/d.diffMap[lv].total*100)),backgroundColor:['#E6E2DA','#CFC9BE','#A8A49B','var(--blue)','#6B675E'],borderRadius:4,borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>c.raw+'%'}}},scales:{y:{min:0,max:100,ticks:{callback:v=>v+'%',stepSize:25},grid:{color:'rgba(128,128,128,0.1)'}},x:{grid:{display:false}}}}});
  ...
  if(gs&&d.grpStats){
    ...
    gs.innerHTML=box('คะแนนเฉลี่ยกลุ่ม',st.avg+'<span style="font-size:11px;color:var(--text3);font-weight:400"> /30</span>','จาก '+st.count+' คน','var(--blue)')
      ...
      +box('สะเพร่าเฉลี่ย',st.careAvg+'<span style="font-size:11px;color:var(--text3);font-weight:400"> ข้อ</span>','ต่อคน','var(--amber)');
  }
...
function renderAllComparison(d){
  ...
  groups.forEach(g=>{
    ...
    chip.style.cssText=`font-size:11px;padding:5px 12px;border-radius:16px;border:1px solid ${on?'var(--blue)':'var(--border-md)'};background:${on?'var(--blue)':'transparent'};color:${on?'#fff':'var(--text2)'};cursor:pointer;font-weight:${isMine?'600':'400'}`;
    chip.onclick=()=>{
  ...
  statsEl.innerHTML=box('อันดับของคุณ',(myRank||'—')+'<span style="font-size:11px;color:var(--text3);font-weight:400"> /'+members.length+'</span>','จาก '+selectedGroups.length+' กลุ่ม','var(--blue)')
    +box('คุณอยู่ Top',topPct+'%','ของทุกคนที่สอบบทนี้',topPct<=25?'var(--green)':topPct<=50?'var(--blue)':'var(--amber)')
    +box('คะแนนคุณ',myScore+'<span style="font-size:11px;color:var(--text3);font-weight:400"> /30</span>',(myScore>=avg?'+':'')+(Math.round((myScore-avg)*10)/10)+' จากเฉลี่ย '+avg,myScore>=avg?'var(--green)':'var(--amber)');
  ...
  const barColors=bins.map((b,i)=>i===myBin?'var(--blue)':'var(--surf)');
...
function renderParentDash(d){
  ...
  const cur=_getPType();
  if(bar){
    bar.innerHTML='<div style="font-size:11px;color:var(--text3,var(--text3));margin-bottom:6px">สไตล์การรายงาน — เลือกแบบที่ใกล้เคียง "สไตล์การเชียร์ลูก" ของคุณ (ระบบจะจำไว้ในเครื่องนี้)</div>'
      +'<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:14px">'
      +Object.entries(PARENT_TYPES).map(([k,a])=>`<button onclick="setPType('${k}')" style="flex:1 1 100px;min-width:96px;border:1.5px solid ${k===cur?'var(--blue)':'var(--border,var(--border-md))'};background:${k===cur?'var(--blue-l,var(--blue-l))':'#fff'};border-radius:10px;padding:7px 4px;cursor:pointer;text-align:center;font-family:inherit"><span style="font-size:20px;display:block">${a.em}</span><b style="font-size:12px">${a.nm}</b><span style="display:block;font-size:9.5px;color:var(--text3,var(--text3))">${a.tag}</span></button>`).join('')
      +'</div>';
  }
...
function _parentDolphin(d){
  ...
  if(totalReview>0)problems.push({icon:'🔁',color:'var(--amber-bg,var(--amber-l))',border:'var(--amber,var(--amber))',count:totalReview,countColor:'var(--amber,var(--amber))',title:'ต้องทบทวนรวม '+totalReview+' ข้อ',desc:'แบ่งเป็น: ⚠️สะเพร่า '+(d.care||0)+' | 🧠คอนเซปต์ '+(d.concept||0)+' | ❌ทำไม่ได้ '+(d.cant||0)+' | ⏰ไม่ทัน '+(d.timeout||0)+' ข้อ'});
  const F=_parentFacts(d);
  if((d.concept||0)>0)problems.push({icon:'🧠',color:'var(--surf)',border:'#55606B',count:(d.concept||0),countColor:'#55606B',title:'คอนเซปต์ยังไม่แน่น '+(d.concept||0)+' ข้อ',desc:_pv([
  ...
  if((d.cant||0)>0)problems.push({icon:'❌',color:'var(--red-l)',border:'#8C2D0A',count:(d.cant||0),countColor:'#8C2D0A',title:'ทำไม่ได้ '+(d.cant||0)+' ข้อ',desc:_pv([
  ...
  if((d.timeout||0)>0)problems.push({icon:'⏰',color:'var(--surf)',border:'#55606B',count:(d.timeout||0),countColor:'#55606B',title:'ไม่ทันเวลา '+(d.timeout||0)+' ข้อ',desc:_pv([
  ...
  // คำแนะนำ
  const actions=[];
  if(F.chronic.length)actions.push({title:'จุดที่พลาดซ้ำหลายครั้ง',color:'var(--amber)',text:_pv([
...
function _parentEagle(d){
  ...
  const td='padding:7px 8px;border-bottom:1px solid var(--border,var(--border-md))';
  const th='text-align:left;padding:6px 8px;background:var(--surf,var(--card));font-size:11.5px;color:var(--text3)';
  ...
  document.getElementById('p-actions').innerHTML=`
    <div style="border-left:4px solid var(--green);background:var(--green-l);padding:11px 13px;border-radius:0 10px 10px 0;margin-bottom:8px;font-size:13px"><b style="display:block;font-size:12px;margin-bottom:3px">✅ ประโยคที่ช่วยลูกได้มากสัปดาห์นี้</b>${_pv(sayPool,d,5)}</div>
    <div style="border-left:4px solid var(--red);background:var(--red-l);padding:11px 13px;border-radius:0 10px 10px 0;font-size:13px"><b style="display:block;font-size:12px;margin-bottom:3px">⛔ ประโยคที่งานวิจัยพบว่าทำให้คะแนนครั้งหน้าแย่ลง</b>${_pv(dontPool,d,6)}</div>`;
  const talkEl=document.getElementById('p-talkList');
...
function _parentElephant(d){
  ...
  document.getElementById('p-summary').innerHTML=`
    <div style="font-size:13px;color:var(--green);font-weight:600;margin-bottom:8px">🐘 ข่าวดีของ${name}ประจำรายงานนี้ 🌱</div>
  ...
  document.getElementById('p-actions').innerHTML=`<div class="action-card" style="border-left-color:var(--green)"><div class="action-title" style="color:var(--green)">สิ่งเดียวที่ช่วยได้มากที่สุด</div><div class="action-text">${_pv([
...
function _parentOwl(d){
  ...
  let st='🟢',color='var(--green)',head;
  if(d.score<15||drop2){ st='🔴';color='var(--red)'; head=_pv([
  ...
  else if(d.delta!=null&&d.delta<0){ st='🟡';color='var(--amber)'; head=_pv([
  ...
  const td='padding:6px 8px;border-bottom:1px solid var(--border,var(--border-md))';
  document.getElementById('p-problems').innerHTML=`
...
function _parentBee(d){
  ...
  document.getElementById('p-summary').innerHTML=`
    <div style="max-width:340px;margin:0 auto;border:1px solid var(--border,var(--border-md));border-radius:14px;overflow:hidden;box-shadow:0 3px 10px rgba(0,0,0,.08)">
      <div style="background:var(--blue);color:#fff;padding:10px 14px;font-size:13px">📊 MathsBankTutor · รายงาน${name}</div>
      <div style="padding:14px">
        ...
        <div style="background:var(--amber-l);border:1px solid var(--amber);border-radius:10px;padding:10px 12px;margin-top:10px;font-size:12.5px">💬 <b style="color:var(--accent)">สิ่งเดียวที่อยากรบกวน — ${ask.t}:</b><br>${ask.b}<br><span style="font-size:11px;color:var(--text3)">${ask.n}</span></div>
      </div>
    </div>`;
...
const _DARK_CSS=`html.force-dark{--bg:#141210;--card:#1E1B17;--border:#37322B;--border-md:#37322B;--text1:#F5F3EF;--text2:#A29D93;--text3:#6B675E;--surf:#262219;--blue:#F5F3EF;--blue-l:#262219;--accent:#FF7A45;--accent-l:#3A1B0E;--amber-l:#33280A;--red-l:#331206;--green-l:#132B21}
...
html.force-light{--bg:#ECEAE6;--card:#FCFBF9;--border:#D8D4CC;--border-md:#D8D4CC;--text1:#17150F;--text2:#6B675E;--text3:#A8A49B;--surf:#F5F3EF;--blue:#17150F;--blue-l:#F5F3EF;--accent:#E8571F;--accent-l:#FFF1EA;--amber-l:#FBF1DA;--red-l:#F7E6DF;--green-l:#E8F3EE}
html.force-light .q-blank{background:repeating-linear-gradient(45deg,var(--surf),var(--surf) 4px,var(--bg) 4px,var(--bg) 8px);color:#777;border:1px dashed #AAA}`;
...
const SUB_STATES = [
  ...
  { v:'✅ ถูก',       short:'✓', label:'ถูก',       bg:'#2D6A4F',      fg:'#fff' },
  { v:'⚠️ สะเพร่า',   short:'ส', label:'สะเพร่า',   bg:'#F2C744',      fg:'#4A3300' },
  { v:'C คอนเซปต์',   short:'C', label:'คอนเซปต์',  bg:'#E8571F',      fg:'#fff' },
  { v:'X ทำไม่ได้',   short:'X', label:'ทำไม่ได้',  bg:'#8C2D0A',      fg:'#fff' },
  { v:'⏰ ไม่ทัน',     short:'⏰', label:'ไม่ทัน',    bg:'#55606B',      fg:'#fff' }
];
  ...
  try{
    ...
    box.innerHTML = j.items.slice().reverse().map(it => `
      <div class="sub-row">
        ...
        <div style="font-family:var(--font-num);font-size:14px;font-weight:600;min-width:44px;text-align:right">${it.score}/30</div>
